...
public class MapActivity extends AppCompatActivity {
    ...
    private void doBindService() {
        bindService(new Intent(MapActivity.this, SocketService.class), mConnection, Context.BIND_AUTO_CREATE);
        mIsBound = true;
                    ...
                    for(int i = 1; i < 5; i++) {
                        if((int)(((float) mSize.x/8f) *(1f + (i-1)*2)) - 50 <= clickCoords.x && (int)(((float) mSize.x/8f) *(1f + (i-1)*2)) + 50 >= clickCoords.x) {
                            ...
                            try {
                                ...
                                Message msg = Message.obtain(null, SocketService.MSG_SET_TARGET_LOCATION);
                                msg.replyTo = mMessenger;
                                //msg.obj = "MOVE,10,:";
